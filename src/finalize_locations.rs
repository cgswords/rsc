...
use expose_frame_variables::Location as EFVLoc;
...
fn loc(input : Location) -> EFVLoc {
  return match input
  { Location::Reg(reg)    => EFVLoc::Reg(reg)
  , Location::FrameVar(n) => EFVLoc::FrameVar(n)
  }
}

fn uvar(uvar: UniqueVar, map: &HashMap<UniqueVar, Location>) -> EFVLoc {
...
fn var(input : Variable, map: &HashMap<UniqueVar, Location>) -> EFVLoc {
  return match input
...
fn offset(input: Offset, map: &HashMap<UniqueVar, Location>) -> EFVOffset {
  return match input
  { Offset::UVar(uv) => if let EFVLoc::Reg(s) = uvar(uv, map) {
                          EFVOffset::Reg(s)
                        } else {
